...
module.exports = function () {
    ...
    this.Given(/^The game has started$/, function (callback) {
        var game = new global.window.TicTacToe();
        assert.equal(game.playcases[0].player(), null, 'The game is not at the inital state.');
        callback();
    });
    ...
    this.Then(/^The hand switch to the adversary$/, function (callback) {
        callback.pending();
        //TODO
        //assert.equal(myList.getAll().length, 1, '');
        //assert.equal(1, 1, '');
        //assert.expect(1).toBe(1); // jasmine-based
        //callback();
    });
